...
import { DEFAULT_TOOLTIP } from "./core/chart-utils";
...
function renderChart(cd) {
    const qualiteOrdre = { tres_bon: 5, bon: 4, passable: 3, mediocre: 2, mauvais: 1 };
    let allAnalyses = cd.points
        .flatMap((pt) => pt.analyses.map((a) => ({ ...a, pointLabel: pointLabel(pt) })))
    ...
    activeChart = new Chart(
        document.getElementById("qualite-chart").getContext("2d"),
        {
            ...
            data: {
                ...
                datasets: [{
                    data: allAnalyses.map((a) => qualiteOrdre[a.qualite] ?? 0),
                    backgroundColor: allAnalyses.map((a) => (QUALITE_CONFIG[a.qualite] || QUALITE_CONFIG.tres_bon).hex),
                    borderRadius: 4,
                }],
            },
            options: {
                ...
                scales: {
                    y: {
                        ...
                        max: 5.5,
                        ticks: {
                            callback: (v) => ["", "Mauvais", "Médiocre", "Passable", "Bon", "Très bon"][v] || "",
                        },
                    },
                },
            },
...
document.addEventListener("click", function (e) {
    ...
    if (btn) {
        e.preventDefault();
        e.stopPropagation();
        ouvrirOverlayHistorique(btn.getAttribute("data-point-id"));
    }
});
...
function ouvrirOverlayHistorique(pointId) {
    ...
    container.innerHTML = pt.analyses.map((a, i) => {
        ...
        ].filter(() => a.type === "bandelette" || a.type === "les_deux");

        const photoFields = [
            ["Phosphate", p.phosphate, "mg/L"], ["Nitrate", p.nitrate, "mg/L"], ["Ammoniaque", p.ammoniaque, "mg/L"],
        ].filter(() => a.type === "photometre" || a.type === "les_deux");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (coursDEaux.length > 0) {
        window.selectCoursDEau(coursDEaux[0].id);
    }
});
