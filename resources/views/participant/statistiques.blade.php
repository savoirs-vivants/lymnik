            ...
            <div class="flex-1 overflow-y-auto">
                    ...
                    <div class="space-y-1" id="mesures-checkboxes">
                        @php
                            $mesuresList = [
                                ...
                                'nitrate_photo' => ['label' => 'Nitrate (photo)', 'unit' => 'mg/L', 'color' => 'bg-indigo-500'],
                            ];
                        @endphp
                        @foreach ($mesuresList as $key => $m)
                            <label class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors group">
                                <input type="checkbox" value="{{ $key }}"
                                    class="filter-mesure w-3.5 h-3.5 rounded accent-[#222a60]" checked>
                                <div class="flex items-center gap-2 flex-1 min-w-0">
                                    <span class="w-2 h-2 rounded-full {{ $m['color'] }} shrink-0"></span>
                                    <span class="text-sm font-medium text-slate-700 group-hover:text-slate-900 truncate">{{ $m['label'] }}</span>
                                </div>
                                @if ($m['unit'])
                ...
                <div class="px-5 py-4 border-b border-slate-100">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex gap-2">
                            <button id="rivers-all" class="text-[10px] text-[#222a60] font-semibold hover:underline">Tout</button>
                            <span class="text-slate-300 text-[10px]">·</span>
                            <button id="rivers-none" class="text-[10px] text-slate-400 font-semibold hover:underline">Aucun</button>
                        </div>
                    </div>
                    <div class="space-y-1 max-h-52 overflow-y-auto" id="rivers-checkboxes">
                        @foreach ($coursDEaux as $cd)
                            <label class="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-slate-50 cursor-pointer transition-colors group">
                                ...
                                <span class="text-sm text-slate-600 group-hover:text-slate-900 truncate">{{ $cd->nom }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="px-5 py-4">
                    ...
                    <div class="grid grid-cols-4 gap-1.5 mb-4">
                        @foreach (['7' => '7J', '30' => '30J', '365' => '1 an', 'all' => 'Tout'] as $range => $label)
                            <button class="filter-time-btn py-2 px-1 rounded-xl text-[11px] font-bold transition-all
        ...
        <div class="flex-1 overflow-y-auto bg-slate-50">
            <div class="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4" id="kpi-row">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_12px_rgba(34,42,96,0.06)] p-4">
                        <p class="text-[9px] font-mono font-bold uppercase tracking-widest text-slate-400 mb-2">Cours d'eau</p>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_12px_rgba(34,42,96,0.06)] p-4 sm:p-6">
                    <div class="flex items-center justify-between mb-4 sm:mb-5">
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400 font-mono mt-0.5 hidden sm:block">Moyenne journalière · tous les cours d'eau sélectionnés</p>
                        </div>
                    </div>
