...
@section('content')
    ...
    <div
        ...
        <aside class="w-full lg:w-80 flex-shrink-0 border-r border-slate-100 bg-white flex flex-col z-10">
        ...
        <div id="analyses-detail" class="hidden lg:flex flex-col flex-1 overflow-y-auto bg-slate-50/50 relative">
            ...
            <div id="detail-panel" class="hidden w-full max-w-7xl mx-auto p-6 lg:p-10 space-y-6">
                <div class="flex items-center justify-between bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <div>
                        <div class="flex items-center gap-4 mb-1">
                            ...
                            <span id="detail-qualite-badge"></span>
                        </div>
                        <p id="detail-meta" class="text-sm text-slate-500 font-mono"></p>
                    </div>
                </div>

                <div id="detail-kpis" class="grid grid-cols-2 md:grid-cols-5 gap-4"></div>

                <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mt-6">
                    <div class="p-6 border-b border-slate-50">
                        <h3 class="text-lg font-bold text-[#222a60]">Analyses par point de mesure</h3>
                    </div>
    ...
    <div id="point-overlay" class="fixed inset-0 z-50 hidden">
        ...
        <div class="absolute right-0 top-0 h-full w-full max-w-xl bg-white shadow-2xl flex flex-col overflow-hidden">
            <div class="flex items-center justify-between px-8 py-6 border-b border-slate-100 bg-slate-50/50">
                <div>
                    <h3 id="overlay-title" class="text-xl font-black text-[#222a60] font-grotesk truncate"></h3>
                    <p id="overlay-subtitle" class="text-xs text-slate-500 font-mono mt-1"></p>
                </div>
                <button onclick="closeOverlay()"
                    class="w-10 h-10 rounded-full bg-white border border-slate-200 hover:bg-slate-100 flex items-center justify-center text-slate-500 transition-colors">
            ...
            <div id="overlay-map" class="w-full h-48 shrink-0 bg-slate-200 relative"></div>
