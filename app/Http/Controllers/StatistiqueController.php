...
class StatistiqueController extends Controller
{
    ...
    public function export(Request $request)
    {
        ...
        if (!empty($rivers)) {
            $query->whereIn('cours_d_eaus.id', $rivers);
        }
        if ($dateStart) {
            $query->whereDate('analyses.created_at', '>=', $dateStart);
        }
        if ($dateEnd) {
            $query->whereDate('analyses.created_at', '<=', $dateEnd);
        }
        ...
        foreach ($analysesRaw as $a) {
            ...
            $qualiteSafe = $a->qualite ?: 'passable';
            ...
            $qKey = strtolower(str_replace(['é', 'è', ' '], ['e', 'e', '_'], $qualiteSafe));
            if (isset($qualiteStats[$a->cours_d_eau_nom][$qKey])) {
                $qualiteStats[$a->cours_d_eau_nom][$qKey]++;
            }
